...
<div class="message-card" data-id="{{ $id }}">
    <p>{!! ($message == null && $attachment != null && @$attachment[2] != 'file') ? $attachment[1] : nl2br($message) !!}
        ...
        <a href="{{route('attachments.download',['fileName'=>$attachment[0]]) }}" style="color: #595959;"
            class="file-download">
...
<div>
    <div class="message-card">
        <div class="image-file chat-image"
            style="width: 250px; height: 150px;background-image: url('{{ asset('storage/'.'chat'.'/'.$attachment[0]) }}')">
        </div>
    </div>
</div>
@endif
{{-- If attachment is an vedio --}}
@if(@$attachment[2] == 'video')
<div>
    <div class="message-card">
        <video style="width: 250px; height: 150px;" controls='controls'>
            <source src="{{ asset('storage/'.'chat'.'/'.$attachment[0]) }}" />
        </video>
    </div>
</div>
@endif
...
<div class="message-card mc-sender" data-id="{{ $id }}">
    <p>{!! ($message == null && $attachment != null && @$attachment[2] != 'file') ? $attachment[1] : nl2br($message) !!}
        ...
        {{-- If attachment is a file --}}
        @if(@$attachment[2] == 'file')
        <a href="{{ route('attachments.download',['fileName'=>$attachment[0]]) }}" class="file-download">
            <span class="fas fa-file"></span> {{$attachment[1]}}</a>
        @endif
    </p>
</div>
